    ...
    if ($stmt->execute()) {
        ...
        if ($rez) {
            ...
            if ($new_password==$password) {
                ...
                $_SESSION['admin_ID'] = $rez['ID'];
                $_SESSION['email_admin'] = $rez['email'];
                $_SESSION['ime_admin'] = $rez['ime'];
                header("Location: admin/index_admin.php"); // Preusmeravanje na željenu stranicu nakon prijave
                exit();
            } else {
                $poruka= "Pogrešna lozinka!";
                header("Location: admin_login.php?error=".$poruka);
            }
        } else {
